...
def upload(source, destination, ttl_days):
    ...
    p = curl(
        '--silent --show-error --include --header "Content-Type: application/json" --data @- {}'.format(
            auth_url
        ),
    ...
    if header_lines[0].split()[1] != "201":
        raise Exception(
            "Failed to log in to Openstack at {}: {}\n{}".format(
    ...
    p = curl(
        "--fail --silent --show-error --head --header @- {}".format(container_url),
    ...
    for path, _, files in os.walk(source):
        ...
        for f in files:
            ...
            dest_url = (full_destination_url + "/" + path + "/" + f).replace("//", "/")
            ...
            try:
                p = curl(
                    '--silent --show-error --head --header @- -o /dev/null --write-out "%{{http_code}}" {}'.format(
                        dest_url
                    ),
                    input=auth_token_header,
                )
